...
<body class="bg-gray-50">
    ...
    <div class="flex min-h-screen">
        <!-- Sidebar -->
        <aside class="w-64 glass border-r border-gray-200/50 hidden lg:block fixed h-full z-40 transition-all duration-300">
            ...
            <nav class="p-4 space-y-1.5 h-[calc(100vh-80px)] overflow-y-auto">
                <a href="{{ route('admin.dashboard') }}"
                    ...
                    <i data-lucide="home" class="w-5 h-5 transition-transform group-hover:scale-110 {{ request()->routeIs('admin.dashboard') ? 'text-indigo-600' : 'text-gray-400 group-hover:text-gray-600' }}"></i>
                    Tableau de Bord
                </a>
                ...
                <a href="{{ route('admin.notes.index') }}"
                    ...
                    <i data-lucide="notebook" class="w-5 h-5 transition-transform group-hover:scale-110 {{ request()->routeIs('admin.notes.*') ? 'text-indigo-600' : 'text-gray-400 group-hover:text-gray-600' }}"></i>
                    Gérer les Notes
                </a>
                ...
                <div class="pt-4 mt-4 border-t border-gray-100">
                    <a href="{{ route('home') }}"
                        ...
                        <i data-lucide="external-link" class="w-5 h-5 text-gray-400 group-hover:text-gray-600 transition-transform group-hover:translate-x-0.5"></i>
                        Voir le Site
                    </a>
                </div>
            </nav>
        ...
        <div class="flex-1 flex flex-col lg:ml-64 transition-all duration-300">
            <!-- Header -->
            <header class="glass border-b border-gray-200/50 h-16 flex items-center justify-between px-6 sticky top-0 z-30">
                <div class="flex items-center gap-4">
                    <button class="lg:hidden text-gray-500 hover:text-gray-700 transition">
                        <i data-lucide="menu" class="w-6 h-6"></i>
                    </button>
                    <h1 class="text-lg font-semibold text-gray-800 tracking-tight">@yield('header-title', 'Tableau de Bord Admin')</h1>
                </div>

                <div class="flex items-center gap-3">
                    <span class="text-sm font-medium text-gray-600">Administrateur</span>
                    <div class="w-9 h-9 rounded-full bg-gradient-to-br from-indigo-100 to-violet-100 border border-indigo-200 flex items-center justify-center shadow-sm">
            ...
            <main class="flex-1 p-6 lg:p-8">
                @if(session('success'))
                    <div class="mb-6 p-4 bg-green-50/50 border border-green-200 text-green-700 rounded-xl flex items-center gap-3 animate-fade-in shadow-sm">
                ...
                @if(session('error'))
                    <div class="mb-6 p-4 bg-red-50/50 border border-red-200 text-red-700 rounded-xl flex items-center gap-3 animate-fade-in shadow-sm">
