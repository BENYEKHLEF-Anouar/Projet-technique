        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Créé le
                    </th>
                        ...
                        <td class="px-6 py-4">
                            <div class="flex flex-wrap gap-1">
                                @foreach($note->categories as $cat)
                                    <span
                                        class="px-2 py-0.5 text-xs rounded-full bg-indigo-50 text-indigo-600 border border-indigo-100">{{ $cat->name }}</span>
                                @endforeach
                            </div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm text-gray-600">{{ $note->user->name ?? 'Inconnu' }}</div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right">
                            <div class="flex justify-end items-center gap-2">
                                <button type="button" onclick="openViewModal({{ $note->id }})"
                                    class="p-2 text-gray-400 hover:text-indigo-600 transition" title="Voir">
                                    <i data-lucide="eye" class="w-4 h-4"></i>
                                </button>
                                <button type="button" onclick="openEditModal({{ $note->id }})"
                                    class="p-2 text-gray-400 hover:text-indigo-600 transition" title="Modifier">
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-6 py-12 text-center">
                            ...
                            <p class="text-sm text-gray-500 mt-1">Essayez d'ajuster vos filtres ou créez une nouvelle note
                            </p>
                        </td>
                    </tr>
                @endforelse
