        ...
        <form id="filter-form" action="{{ route('home') }}" method="GET" class="relative">
            <div class="flex flex-col sm:flex-row gap-3">
                ...
                <a href="{{ route('home') }}" id="clear-filters"
                    class="inline-flex items-center justify-center gap-x-2 py-3 px-4 rounded-lg border border-transparent text-sm font-semibold text-gray-500 hover:bg-gray-100 disabled:opacity-50 disabled:pointer-events-none transition {{ (!request('search') && !request('category')) ? 'hidden' : '' }}">
